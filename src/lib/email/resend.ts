...
export async function sendLeadNotification(opts: {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      subject: `New Setu Lead: ${opts.company ?? opts.leadEmail}`,
      html: `
        <h2>New Blueprint Lead</h2>
        <table style="border-collapse:collapse;width:100%">
          <tr><td style="padding:8px;font-weight:bold">Email</td><td style="padding:8px">${opts.leadEmail}</td></tr>
          ${opts.leadName ? `<tr><td style="padding:8px;font-weight:bold">Name</td><td style="padding:8px">${opts.leadName}</td></tr>` : ""}
          ${opts.company ? `<tr><td style="padding:8px;font-weight:bold">Company</td><td style="padding:8px">${opts.company}</td></tr>` : ""}
          ${opts.inputSummary ? `<tr><td style="padding:8px;font-weight:bold">Workflow</td><td style="padding:8px">${opts.inputSummary}</td></tr>` : ""}
...
export async function sendBlueprintReviewNotification(opts: {
  ...
  try {
    ...
    await resend.emails.send({
      from: env.FROM_EMAIL,
      to: env.ADMIN_ALERT_EMAIL,
      subject: `Blueprint Ready for Review: ${opts.agentName ?? "Unknown Agent"}`,
      html: `
        <h2>Blueprint Pending Review</h2>
        <table style="border-collapse:collapse;width:100%">
          <tr><td style="padding:8px;font-weight:bold">Workflow</td><td style="padding:8px">${opts.inputSummary}</td></tr>
          ${opts.agentName ? `<tr><td style="padding:8px;font-weight:bold">Recommended Agent</td><td style="padding:8px">${opts.agentName}</td></tr>` : ""}
