...
export type ToolCategory =
  ...
  | 'Compliance & GRC'
  | 'Data Warehouse'
  // This batch came from a different audit than the six above: instead of
  // guessing at plausible categories, grepped every tool name literally
  // referenced across all 102 employee profiles' own `tools` arrays (783
  // distinct strings) and diffed against the registry. These are real
  // claims real profiles already make that had no connectable tool behind
  // them at all — not hypothetical coverage, confirmed gaps.
  | 'Community'
  | 'Customer Success'
...
export const TOOL_REGISTRY: ToolDef[] = [
  ...
  // ── Data & BI, more depth ──────────────────────────────────────────────────
  {
    ...
    domain: 'metabase.com',
    category: 'Data & BI',
    description: 'Open-source BI, self-hosted or cloud — the most-referenced BI tool across this app\'s own employee profiles, and it had no entry. Questions, dashboards, cards. API base: https://{host}/api — host is your Metabase instance, set as config when connecting.',
    authType: 'api_key',
    authLabel: 'API Key',
    authPlaceholder: 'mb_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Metabase → Admin Settings → Authentication → API Keys → Create API Key',
    baseUrl: 'https://{host}/api',
    docsUrl: 'https://www.metabase.com/docs/latest/api-documentation',
  },
  {
    slug: 'powerbi',
    name: 'Power BI',
    ...
    category: 'Data & BI',
    description: 'Microsoft\'s BI tool, competitor to Tableau/Looker. Datasets, reports, dashboards, refresh. API base: https://api.powerbi.com/v1.0/myorg. Same Azure AD OAuth2 app as Microsoft 365.',
    authType: 'bearer',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'eyJ0eXAiOiJKV1QiLCJhbGc...',
    authHint: 'Same Azure app registration as Microsoft 365, with Power BI Service permissions granted',
    baseUrl: 'https://api.powerbi.com/v1.0/myorg',
    docsUrl: 'https://learn.microsoft.com/en-us/rest/api/power-bi/',
  },
  // ── Analytics, more depth ──────────────────────────────────────────────────
  {
    slug: 'heap',
    name: 'Heap',
    domain: 'heap.io',
    category: 'Analytics',
    description: 'Auto-captured product analytics, competitor to Mixpanel/Amplitude. Query API for events/users, Track API for server-side events. API base: https://heapanalytics.com/api. Bearer token (App ID + API key pair).',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Heap → Account Settings → Projects → API Keys → Create API Key',
    baseUrl: 'https://heapanalytics.com/api',
    docsUrl: 'https://developers.heap.io/reference/api-overview',
  },
  {
    slug: 'triple-whale',
    name: 'Triple Whale',
    ...
    category: 'Analytics',
    description: 'E-commerce/DTC analytics — blended ad spend, LTV, attribution across every channel in one number. API base: https://api.triplewhale.com/api/v2.',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Triple Whale → Settings → API → Generate API Key',
  ...
  },
  {
    ...
    category: 'Analytics',
    description: 'A/B testing and feature experimentation. Experiments, audiences, results. API base: https://api.optimizely.com/v2. Bearer token (Personal Access Token).',
    authType: 'bearer',
    authLabel: 'Personal Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Optimizely → Profile → Personal Access Tokens → Generate New Token',
    baseUrl: 'https://api.optimizely.com/v2',
    docsUrl: 'https://library.optimizely.com/docs/api/app/v2/index.html',
  },
  // ── Content & Docs, more depth ─────────────────────────────────────────────
  {
    slug: 'confluence',
    name: 'Confluence',
    domain: 'atlassian.com',
    category: 'Content & Docs',
    description: 'Atlassian wiki/docs, pairs with the existing Jira entry — same account, same auth. Pages, spaces, attachments. API base: https://{domain}.atlassian.net/wiki/rest/api. Basic auth with email:api_token.',
    authType: 'basic',
    authLabel: 'Email + API Token',
    authPlaceholder: 'email@company.com:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Atlassian → id.atlassian.com → Security → Create and manage API tokens (same token works for Jira and Confluence)',
    baseUrl: 'https://{domain}.atlassian.net/wiki/rest/api',
  ...
  },
  {
    ...
    domain: 'loom.com',
    category: 'Content & Docs',
    description: 'Async video messaging — record and share a video instead of a long doc. Video metadata, embed, folder management. Public REST access is more limited than most tools here (mostly workspace/enterprise-gated) — API base: https://www.loom.com/v1/api. OAuth2.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Loom → Settings → Developers (workspace plan required for API access) → create an app',
    baseUrl: 'https://www.loom.com/v1/api',
  ...
  },
  {
    ...
    domain: 'miro.com',
    category: 'Content & Docs',
    description: 'Collaborative whiteboarding. Boards, items (sticky notes, shapes, frames), members. API base: https://api.miro.com/v2. OAuth2 or a dev-team access token.',
    authType: 'bearer',
    authLabel: 'Access Token',
    authPlaceholder: 'eyJhbGciOiJIUzI1NiJ9...',
    authHint: 'Miro → Settings → Your apps → Create new app → OAuth2 flow, or grab a dev token for one board',
  ...
  },
  {
    ...
    domain: 'getguru.com',
    category: 'Content & Docs',
    description: 'Internal knowledge base with verification/staleness tracking. Cards, collections, boards. API base: https://api.getguru.com/api/v1. Basic auth with your Guru email as username, user token as password.',
    authType: 'basic',
    authLabel: 'Email + User Token',
    authPlaceholder: 'you@company.com:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Guru → your profile icon → generate a User Token',
  ...
  },
  {
    ...
    domain: 'box.com',
    category: 'Content & Docs',
    description: 'Enterprise file storage, alternative to Google Drive/OneDrive at companies with stricter compliance requirements. Files, folders, sharing. API base: https://api.box.com/2.0. OAuth2.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Box Developer Console → Create Platform App → OAuth2 flow',
    baseUrl: 'https://api.box.com/2.0',
    docsUrl: 'https://developer.box.com/reference/',
  },
  // ── Messaging, more depth ──────────────────────────────────────────────────
  {
    slug: 'wati',
    name: 'WATI',
    domain: 'wati.io',
    category: 'Messaging',
    description: 'WhatsApp Business team inbox — very common with Indian SMBs as a friendlier layer over the raw WhatsApp Business API (which is also in this registry). Contacts, templates, broadcast, chat. API base: https://live-server-{instance}.wati.io/api/v1 — instance set as config when connecting.',
    authType: 'bearer',
    authLabel: 'API Access Token',
    authPlaceholder: 'eyJhbGciOiJIUzI1NiJ9...',
    authHint: 'WATI → API Docs (in-app) → copy your Access Token and instance URL',
    baseUrl: 'https://live-server-{instance}.wati.io/api/v1',
    docsUrl: 'https://docs.wati.io/reference/introduction',
  },
  // ── Social Media, more depth ───────────────────────────────────────────────
  {
    slug: 'meta-business-suite',
    name: 'Meta Business Suite',
    domain: 'business.facebook.com',
    category: 'Social Media',
    description: 'Organic Facebook/Instagram posting and Page management — distinct from the existing meta-ads entry, which is paid only. Posts, comments, insights, Page management. API base: https://graph.facebook.com/v19.0. OAuth2 Page Access Token.',
    authType: 'bearer',
    authLabel: 'Page Access Token',
    authPlaceholder: 'EAAxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Meta for Developers → your app → Graph API Explorer → generate a Page Access Token with pages_manage_posts',
  ...
  },
  {
    ...
    category: 'Social Media',
    description: 'Social media management and analytics, competitor to Buffer/Hootsuite. Publishing, engagement, analytics across networks. API base: https://api.sproutsocial.com/v1. OAuth2.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Sprout Social → Business plan or higher → Settings → API Access',
  ...
  },
  {
    ...
    domain: 'later.com',
    category: 'Social Media',
    description: 'Visual social media scheduling, popular for Instagram-heavy brands. Public REST API access is limited (mostly partner-gated) compared to Buffer/Hootsuite — describe what you need clearly and expect some actions may not be reachable.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Later → Settings → Integrations & API (availability varies by plan)',
    baseUrl: 'https://api.later.com/v1',
    docsUrl: 'https://later.com/',
  },
  // ── Community ──────────────────────────────────────────────────────────────
  {
    slug: 'circle',
    ...
    domain: 'circle.so',
    category: 'Community',
    description: 'Paid/branded community platform (courses, discussions, events, memberships). Members, spaces, posts, events. API base: https://app.circle.so/api/v1 (Admin API — headless API is newer/broader).',
    authType: 'bearer',
    authLabel: 'Admin API Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Circle → Settings → Admin API → Generate Token',
  ...
  },
  {
    ...
    domain: 'discord.com',
    category: 'Community',
    description: 'Community/support servers, common for dev tools and gaming-adjacent products. Channels, messages, members, roles. API base: https://discord.com/api/v10. Bot token auth — sent as "Authorization: Bot {token}", not raw Bearer.',
    authType: 'bearer',
    authLabel: 'Bot Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Discord Developer Portal → Applications → your app → Bot → Reset Token → invite the bot to your server',
    baseUrl: 'https://discord.com/api/v10',
    docsUrl: 'https://discord.com/developers/docs/intro',
  },
  // ── Customer Success ────────────────────────────────────────────────────────
  // Distinct from Customer Support (Zendesk/Intercom/Freshdesk/Gorgias/Help
  // Scout, all ticketing) — this is proactive account health, expansion,
  // and churn risk for a CS/AM employee managing a book of accounts, not
  // responding to inbound tickets.
  {
    slug: 'gainsight',
    name: 'Gainsight',
    domain: 'gainsight.com',
    category: 'Customer Success',
    description: 'Enterprise customer success platform. Health scores, timelines, calls-to-action, success plans. API base: https://{domain}.gainsightcloud.com/v1.',
    authType: 'bearer',
    authLabel: 'Access Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Gainsight → Administration → API Access Keys → Create Access Key',
  ...
  },
  {
    ...
    domain: 'planhat.com',
    category: 'Customer Success',
    description: 'Customer success platform, common at mid-market SaaS. Companies, health scores, NPS, tasks, conversations. API base: https://api.planhat.com. Bearer token.',
    authType: 'bearer',
    authLabel: 'API Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Planhat → Settings → API → Generate Token',
    baseUrl: 'https://api.planhat.com',
    docsUrl: 'https://docs.planhat.com/',
  },
  {
    slug: 'churnzero',
    name: 'ChurnZero',
    ...
    category: 'Customer Success',
    description: 'Customer success + churn prevention, SMB/mid-market focused. Accounts, health scores, journeys, playbooks. Event/data API base: https://analytics.churnzero.net.',
    authType: 'api_key',
    authLabel: 'App Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'ChurnZero → Admin → API → App Key',
    baseUrl: 'https://analytics.churnzero.net',
    docsUrl: 'https://help.churnzero.com/hc/en-us/categories/360002378753-API-Integrations',
  },
  // ── Revenue Intelligence ───────────────────────────────────────────────────
  // Call recording, coaching, and forecasting for sales orgs — none of it
  // overlaps with Sales Outreach (Outreach/Salesloft/Apollo, which is
  // sequencing and prospecting, not what happens once someone's on a call).
  {
    slug: 'gong',
    name: 'Gong',
    domain: 'gong.io',
    category: 'Revenue Intelligence',
    description: 'Call recording, transcription, and deal intelligence — very common at B2B sales orgs. Calls, transcripts, users, deals. API base: https://api.gong.io/v2. Basic auth with Access Key as username, Access Key Secret as password.',
    authType: 'basic',
    authLabel: 'Access Key + Secret',
    authPlaceholder: 'ACCESS_KEY:ACCESS_KEY_SECRET',
    authHint: 'Gong → Company Settings → API → Create',
  ...
  },
  {
    ...
    domain: 'chorus.ai',
    category: 'Revenue Intelligence',
    description: 'Conversation intelligence, part of ZoomInfo — competitor to Gong. Calls, transcripts, moments, trackers. Public API access is more limited/partner-gated than Gong\'s — describe what you need clearly.',
    authType: 'bearer',
    authLabel: 'API Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Chorus/ZoomInfo → contact your account team for API access — not fully self-serve',
    baseUrl: 'https://chorus.ai/api',
    docsUrl: 'https://chorus.ai/',
  },
  {
    slug: 'clari',
    name: 'Clari',
    domain: 'clari.com',
    category: 'Revenue Intelligence',
    description: 'Revenue forecasting and pipeline inspection for sales leadership. Forecasts, opportunities, activity capture. API access is enterprise/partner-gated, not a fully public self-serve developer program — treat as a harder integration than most tools here.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Clari → contact your Clari account team to request API/integration access',
    baseUrl: 'https://api.clari.com',
    docsUrl: 'https://www.clari.com/',
  },
  // ── Accounting, more depth ─────────────────────────────────────────────────
  {
    slug: 'netsuite',
    name: 'NetSuite',
    domain: 'netsuite.com',
    category: 'Accounting',
    description: 'Enterprise ERP/accounting (Oracle). GL, invoices, customers, vendors, financial reports. REST API base: https://{account}.suitetalk.api.netsuite.com/services/rest/record/v1 — account set as config when connecting. Token-Based Authentication (a 4-part OAuth1-style credential: consumer key/secret + token/secret).',
    authType: 'oauth',
    authLabel: 'Token-Based Auth Credential',
    authPlaceholder: 'CONSUMER_KEY:CONSUMER_SECRET:TOKEN:TOKEN_SECRET',
    authHint: 'NetSuite → Setup → Integration → Manage Integrations, then Setup → Users/Roles → Access Tokens',
  ...
  },
  {
    ...
    domain: 'mosaic.tech',
    category: 'Accounting',
    description: 'FP&A / financial planning, pulls from your accounting + HRIS + CRM into one model. Metrics, reports, headcount plans. API access is newer and more limited than the established accounting tools here.',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Mosaic → Settings → Integrations → API (ask your Mosaic CSM if not visible — still rolling out)',
    baseUrl: 'https://api.mosaic.tech/v1',
    docsUrl: 'https://www.mosaic.tech/',
  },
  {
    slug: 'tally',
    ...
    category: 'Accounting',
    description: 'The dominant Indian SMB accounting/GST software — referenced across GST, AP, and finance employee profiles in this app. Fundamentally different from every other tool here: Tally usually runs ON-PREMISE on the customer\'s own machine/LAN, exposing an XML-over-HTTP API locally, not a public cloud endpoint. API base: http://{tally_host}:9000 — tally_host is the customer\'s local Tally server address (e.g. their office network), set as config when connecting. Only reachable if that host is network-accessible from wherever this runs.',
    authType: 'api_key',
    authLabel: 'Not applicable — local network access only',
    authPlaceholder: 'n/a',
    authHint: 'Tally → Gateway of Tally → F1 (Help) → Settings → Connectivity → enable ODBC/XML Server on port 9000, and ensure that port is reachable from wherever this runs',
    baseUrl: 'http://{tally_host}:9000',
    docsUrl: 'https://help.tallysolutions.com/tally-prime/api-xml/',
  },
  // ── E-signature, more depth ────────────────────────────────────────────────
  {
    slug: 'zoho-sign',
    name: 'Zoho Sign',
    domain: 'zoho.com',
    category: 'E-signature',
    description: 'E-signature, natural fit for companies already in the Zoho ecosystem (Zoho CRM/Books already in this registry — same account, same OAuth app). Document requests, templates, status. API base: https://sign.zoho.com/api/v1.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Same Zoho API Console app as Zoho CRM/Books, with ZohoSign scopes added',
    baseUrl: 'https://sign.zoho.com/api/v1',
    docsUrl: 'https://www.zoho.com/sign/api/getting-started.html',
  },
  {
    slug: 'ironclad',
    name: 'Ironclad',
    ...
    category: 'E-signature',
    description: 'Contract lifecycle management — beyond signing: intake workflows, clause libraries, obligation tracking. Common at companies whose legal/ops team outgrew plain e-signature. Workflows, records, signatures. API base: https://ironcladapp.com/public/api/v1. OAuth2 client credentials.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Ironclad → Settings → Developers → create an API client',
    baseUrl: 'https://ironcladapp.com/public/api/v1',
    docsUrl: 'https://developer.ironcladapp.com/',
  },
  // ── HR & People, more depth ────────────────────────────────────────────────
  {
    slug: 'lattice',
    name: 'Lattice',
    domain: 'lattice.com',
    category: 'HR & People',
    description: 'Performance management, 1:1s, goals (OKRs), engagement surveys. Distinct from BambooHR/Rippling/Darwinbox, which are core HRIS/payroll — this is the performance-and-engagement layer on top. Public API is newer/more limited than the core HRIS tools here.',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Lattice → Settings → API (Enterprise plans; ask your Lattice CSM if not visible)',
  ...
  },
  {
    ...
    category: 'HR & People',
    description: 'Employee engagement surveys, eNPS, performance reviews. Survey results, people data. Public API access is limited/partner-gated compared to core HRIS tools.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Culture Amp → contact your Culture Amp account team about API/integration access',
    baseUrl: 'https://api.cultureamp.com',
    docsUrl: 'https://www.cultureamp.com/',
  },
  {
    slug: 'keka',
    name: 'Keka',
    domain: 'keka.com',
    category: 'HR & People',
    description: 'Indian HRIS + payroll — very common at Indian startups/SMBs, pairs with the Tally/Volopay/GST-focused finance tooling elsewhere in this registry. Employees, attendance, leave, payroll. API base: https://{company}.keka.com/api/v1 — company subdomain set as config when connecting. OAuth2.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Keka → Settings → Keka API → Generate Client ID and Secret → OAuth2 flow',
    baseUrl: 'https://{company}.keka.com/api/v1',
    docsUrl: 'https://www.keka.com/developer',
  },
  // ── Payroll & Expenses, more depth ─────────────────────────────────────────
  {
    slug: 'volopay',
    name: 'Volopay',
    domain: 'volopay.com',
    category: 'Payroll & Expenses',
    description: 'Corporate cards + expense management for Indian/APAC companies — the regional alternative to Ramp/Brex. Cards, transactions, reimbursements, bill pay. API base: https://api.volopay.com/v1.',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Volopay → Settings → Developers → API Keys → Generate',
    baseUrl: 'https://api.volopay.com/v1',
    docsUrl: 'https://volopay.com/',
  },
  {
    slug: 'coupa',
    name: 'Coupa',
    domain: 'coupa.com',
    category: 'Payroll & Expenses',
    description: 'Enterprise procurement and spend management — purchase orders, invoices, approvals, supplier management. API base: https://{instance}.coupahost.com/api — instance set as config when connecting. OAuth2.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Coupa → Setup → Integrations → OAuth2 Clients → create a client',
    baseUrl: 'https://{instance}.coupahost.com/api',
    docsUrl: 'https://compass.coupa.com/en-us/products/product-documentation/integration-technical-documentation/the-coupa-core-api',
  },
  // ── Engineering, more depth ────────────────────────────────────────────────
  {
    ...
    category: 'Engineering',
    description: 'Feature flags and progressive rollouts. Flags, targeting rules, environments, experiments. API base: https://app.launchdarkly.com/api/v2.',
    authType: 'bearer',
    authLabel: 'Access Token',
    authPlaceholder: 'api-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx',
    authHint: 'LaunchDarkly → Account Settings → Authorization → Access tokens → Create token',
  ...
  },
  {
    ...
    category: 'Engineering',
    description: 'Incident response and on-call management, pairs with the existing Sentry/Datadog entries for a full detect-alert-respond loop. Incidents, services, on-call schedules. API base: https://api.pagerduty.com. Auth header format is "Authorization: Token token={key}" — not raw Bearer.',
    authType: 'api_key',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxx',
    authHint: 'PagerDuty → Integrations → API Access Keys → Create New API Key',
  ...
  },
  {
    ...
    domain: 'okta.com',
    category: 'Engineering',
    description: 'Identity and SSO management. Users, groups, apps, access reviews — also directly useful for the Compliance & GRC employees\' access-review watch patterns. API base: https://{domain}.okta.com/api/v1. Auth header format is "Authorization: SSWS {token}" — not Bearer or Basic.',
    authType: 'api_key',
    authLabel: 'API Token',
    authPlaceholder: '00xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Okta Admin → Security → API → Tokens → Create Token',
    baseUrl: 'https://{domain}.okta.com/api/v1',
    docsUrl: 'https://developer.okta.com/docs/reference/',
  },
  // ── Sales Outreach, more depth ─────────────────────────────────────────────
  {
    slug: 'zoominfo',
    name: 'ZoomInfo',
    domain: 'zoominfo.com',
    category: 'Sales Outreach',
    description: 'B2B contact and company data, competitor/complement to Apollo (already in this registry). Contact search, company search, intent signals. API base: https://api.zoominfo.com. Bearer token obtained via username + client secret exchange.',
    authType: 'bearer',
    authLabel: 'Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'ZoomInfo → Admin Portal → API → generate a client secret, exchange for a token via /authenticate',
  ...
  },
  {
    ...
    domain: 'highspot.com',
    category: 'Sales Outreach',
    description: 'Sales enablement content management — the right pitch deck/case study for the right deal stage. Spots (content), analytics, pitches. API base: https://api.highspot.com/v1.0. Basic auth with Key + Secret.',
    authType: 'basic',
    authLabel: 'Key + Secret',
    authPlaceholder: 'KEY:SECRET',
    authHint: 'Highspot → Admin → API Keys → Create Key',
    baseUrl: 'https://api.highspot.com/v1.0',
    docsUrl: 'https://api-doc.highspot.com/',
  },
  {
    slug: '6sense',
    name: '6sense',
    domain: '6sense.com',
    category: 'Sales Outreach',
    description: 'Intent data and account-based marketing (ABM) — surfaces accounts actively researching your category before they ever fill out a form. Segments, predictions, engagement. API access is enterprise/partner-gated rather than a fully public self-serve program.',
    authType: 'bearer',
    authLabel: 'API Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: '6sense → contact your account team about API access',
  ...
  // ── Referral & Affiliate ────────────────────────────────────────────────────
  {
    ...
    description: 'Partner/affiliate program management — recruit partners, track referrals, pay commissions. Partners, transactions, rewards. API base: https://api.partnerstack.com/api/v2. Basic auth with Public Key as username, Secret Key as password.',
    authType: 'basic',
    authLabel: 'Public Key + Secret Key',
    authPlaceholder: 'PUBLIC_KEY:SECRET_KEY',
    authHint: 'PartnerStack → Settings → API → copy Public Key and Secret Key',
    baseUrl: 'https://api.partnerstack.com/api/v2',
    docsUrl: 'https://docs.partnerstack.com/reference/getting-started',
  },
  {
    slug: 'referralhero',
    name: 'ReferralHero',
    domain: 'referralhero.com',
    category: 'Referral & Affiliate',
    description: 'Referral program management for waitlists/growth campaigns. Campaigns, referrers, referrals, rewards. API base: https://app.referralhero.com/api/v1.',
    authType: 'api_key',
    authLabel: 'API Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'ReferralHero → Campaign Settings → API → copy your API Token',
    baseUrl: 'https://app.referralhero.com/api/v1',
    docsUrl: 'https://www.referralhero.com/api-documentation',
  },
  // ── Design, more depth ──────────────────────────────────────────────────────
  {
    slug: 'adobe-express',
    ...
    domain: 'adobe.com',
    category: 'Design',
    description: 'Templated design tool, competitor to Canva. Public automation API is limited compared to Canva/Figma (mostly an embed SDK for editing-in-your-app, not a headless generate-and-export API) — treat as a harder integration than the other Design entries.',
    authType: 'oauth',
    authLabel: 'OAuth2 Access Token',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Adobe Developer Console → Create Project → Adobe Express Embed SDK / APIs',
    baseUrl: 'https://express.adobe.io',
    docsUrl: 'https://developer.adobe.com/express/',
  },
  // ── Marketing Automation, more depth ───────────────────────────────────────
  {
    slug: 'jasper',
    name: 'Jasper',
    domain: 'jasper.ai',
    category: 'Marketing Automation',
    description: 'AI content generation for marketing copy — ad copy, blog drafts, email variants at scale. Documents, templates, brand voice. API base: https://api.jasper.ai/v1.',
    authType: 'bearer',
    authLabel: 'API Key',
    authPlaceholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
    authHint: 'Jasper → Settings → API Keys (Business plan) → Create Key',
...
export const TOOL_NAME_TO_SLUG: Record<string, string> = {
  ...
  'Metabase': 'metabase', 'Power BI': 'powerbi', 'PowerBI': 'powerbi',
  'Heap': 'heap', 'Triple Whale': 'triple-whale', 'Optimizely': 'optimizely',
  'Confluence': 'confluence', 'Loom': 'loom', 'Miro': 'miro', 'Guru': 'guru', 'Box': 'box',
  'WATI': 'wati', 'Meta Business Suite': 'meta-business-suite',
  'Sprout Social': 'sprout-social', 'Later': 'later',
  'Circle': 'circle', 'Discord': 'discord',
  'Gainsight': 'gainsight', 'Planhat': 'planhat', 'ChurnZero': 'churnzero',
  'Gong': 'gong', 'Chorus': 'chorus', 'Clari': 'clari',
  'NetSuite': 'netsuite', 'Mosaic': 'mosaic', 'Tally': 'tally',
  'Zoho Sign': 'zoho-sign', 'Ironclad': 'ironclad',
  'Lattice': 'lattice', 'Culture Amp': 'culture-amp', 'Keka': 'keka',
  'Volopay': 'volopay', 'Coupa': 'coupa',
  'LaunchDarkly': 'launchdarkly', 'PagerDuty': 'pagerduty', 'Okta': 'okta',
  'ZoomInfo': 'zoominfo', 'Highspot': 'highspot', '6sense': '6sense',
  'PartnerStack': 'partnerstack', 'ReferralHero': 'referralhero',
  'Adobe Express': 'adobe-express', 'Jasper': 'jasper',
  // The single highest-value alias batch: every Google Workspace / Microsoft
  // 365 app referenced by its own product name in profile prose (Gmail,
  // Google Sheets, Excel, ...) resolves to ONE registry entry each, since
  // that's genuinely one connected credential covering the whole suite —
  // not 7 separate "advisory only" mislabels for what is really one Google
  // OAuth connection.
  'Google Sheets': 'google-workspace', 'Google Docs': 'google-workspace',
  'Google Drive': 'google-workspace', 'Gmail': 'google-workspace',
  'Google Calendar': 'google-workspace', 'Google Slides': 'google-workspace',
  'Google Forms': 'google-workspace',
  'Excel': 'microsoft-365', 'SharePoint': 'microsoft-365', 'Outlook': 'microsoft-365',
