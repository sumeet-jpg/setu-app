...
export default async function ApprovalsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  try { approvals = await getApprovals({ status: status ?? "pending" }); } catch { approvals = []; }
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex gap-2">
        {filters.map((f) => (
          <a key={f.value} href={`/admin/approvals?status=${f.value}`}
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${(status ?? "pending") === f.value ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-primary/10"}`}>
            {f.label}
          </a>
        ))}
      </div>
      {approvals.length === 0 ? (
        ...
        <div className="space-y-3">
          {approvals.map((ap) => (
            <div key={ap.id as string} className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  {ap.status === "pending" && <ApprovalActions approvalId={ap.id as string} />}
                </div>
              </div>
              {ap.review_notes && <p className="mt-3 text-xs text-muted-foreground border-t border-border pt-3">Notes: {ap.review_notes as string}</p>}
            </div>
          ))}
        </div>
      )}
