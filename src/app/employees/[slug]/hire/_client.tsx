    ...
    return (
      <div style={{ minHeight: '100vh', background: BG, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-inter)', color: '#fff' }}>
        <div style={{ textAlign: 'center', maxWidth: 480, padding: '0 24px' }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 12px' }}>{e.name} is on their way!</h1>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <Link href="/employees" style={{ padding: '10px 20px', borderRadius: 10, background: SURFACE, border: `1px solid ${BORDER}`, color: '#a1a1aa', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>Browse more employees</Link>
            <Link href={`/employees/${e.slug}/interview`} style={{ padding: '10px 20px', borderRadius: 10, background: '#6366f1', color: '#fff', fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>Chat with {e.name}</Link>
          </div>
        </div>
      </div>
    )
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '48px 24px', display: 'grid', gridTemplateColumns: '1fr 320px', gap: 32, alignItems: 'start' }}>
        <div>
          <div style={{ marginBottom: 32 }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 8px', color: '#fff' }}>Hire {e.name} as your {e.title}</h1>
            <p style={{ fontSize: 14, color: MUTED, margin: 0, lineHeight: 1.6 }}>Fill out this form and Sumeet will reach out within 24 hours. No credit card needed to start.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <p style={{ fontSize: 11, color: '#3f3f46', textAlign: 'center', margin: 0 }}>No credit card required. Sumeet reaches out within 24 hours.</p>
          </form>
        </div>
        ...
        <div style={{ position: 'sticky', top: 24 }}>
          ...
          <div style={{ marginTop: 12, padding: '12px 16px', borderRadius: 12, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', fontSize: 12, color: '#818cf8', lineHeight: 1.6 }}>
            Not sure yet? <Link href={`/employees/${e.slug}/interview`} style={{ color: '#a78bfa', fontWeight: 600, textDecoration: 'none' }}>Interview {e.name} free</Link> — no commitment.
          </div>
        </div>
      </div>
    </div>
